...
func executeLine(k *libkademlia.Kademlia, line string) (response string) {
	...
	case toks[0] == "vanish":
		if len(toks) != 6 {
			response = "length is not enough"
			return
		}
		vdoId := libkademlia.NewRandomID()
		numkey, _ := strconv.Atoi(toks[2])
		threshold, _ := strconv.Atoi(toks[3])
		timeout, _ := strconv.Atoi(toks[4])
		k.Vanish(vdoId, []byte(toks[1]), byte(numkey), byte(threshold), int(timeout))
		response = fmt.Sprintf("Vanish OK")
	case toks[0] == "unvanish":
		...
		nodeId, err := libkademlia.IDFromString(toks[1])
		if err != nil {
			response = "invalid nodeId "
			return
		}
		contact, err := k.FindContact(nodeId)
		if err != nil {
			response = "Unable to find contact "
			return
		}
		vdoId, err := libkademlia.IDFromString(toks[2])
		if err != nil {
			response = " invalid vdoId "
			return
		}
		k.Unvanish(contact.NodeID, vdoId)
		response = fmt.Sprintf("Unvanish OK")
